...
int loadDemoData(NVGcontext* vg, DemoData* data)
{
	...
	data->input_svg = nsvgParseFromFile("../example/bricks.svg", "px", 96.0f);
	if ( data->input_svg == NULL ){
		printf("Error laoding svg");
	}
...
static NVGpaint getPaint(NVGcontext* vg, NSVGpaint* p) {
	...
	if (p->type == NSVG_PAINT_LINEAR_GRADIENT)
		paint = nvgLinearGradient(vg, sx, sy, ex, ey, icol, ocol);
	else
		paint = nvgRadialGradient(vg, sx, sy, 0.0, 160, icol, ocol);
	return paint;
}
